        ...
        if True:
            ...
            if db_entrys:
                ...
                for entry in [entry for entry in db_entrys if entry]:
                    ...
                    Query.add_values(metric_id,entry[0],entry[1])
                #except:
                #    print('writing values failed')
            else:
                Query.pause_basic(metric_id)
                ...
                if db_entrys:
                    try:
                        for entry in db_entrys:
                            print(entry)
                            Query.add_values(metric_id,entry[0],entry[1],datetime.datetime())
                    except:
                        print('writing values failed')
                else:
                    print('BAD FORMAT')
                    Query.pause_key(metric_id)
            except:
                Query.pause_key(metric_id)
                print("FILTER FAILED")
...
def request_token(metric_id,url,value,tag,token_url,key,secret,content_type,auth_type):
